...
var method = 'GET', url = `${apiFileBaseUrl}menu.json`;
...
comment.addEventListener('keydown', () => {
    if ( comment.value.length >= maxLimit ) remaining.innerText = "Max Limit Reached";
    else remaining.innerText = `(${maxLimit - comment.value.length} characters remaining)`;
});
